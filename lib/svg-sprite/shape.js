...
SVGShape.prototype._determineDimensions = function(cb) {
	...
	if (!this.width || !this.height) {
		var that				= this;
		var _browser, _page, _nodeHandle;
		try {
			puppeteer.launch()
			...
			.then(function() {
				return _page.goto('file://dummy.svg');
			})
			.then(function() {
				var svg = that.getSVG(false);
				return _page.setContent(
					//'<svg xmlns="http://www.w3.org/2000/svg">' + 
					'<?xml version="1.0" encoding="UTF-8"?>' + 
					'<svg ' + svg.substr(svg.toLowerCase().indexOf('<svg') + 4) + '</svg>',
			...
			.then(function(nodeHandle) {
				...
				return _page.evaluate(function(node) {
					var rect = node.getBoundingClientRect();
					return { width: rect.width, height: rect.height };
				}, nodeHandle);
			})
			.then(function(bounds) {
				that.width = that._round(bounds.width);
				that.height = that._round(bounds.height);
				return _nodeHandle.dispose();
			})
			.then(function() {
