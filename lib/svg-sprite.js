...
const BrowserManager = require('./browser-mananger');
...
function SVGSpriter(config) {
    ...
    this._queue.on('empty', this._closeBrowser.bind(this));
    ...
    if (!this._queue.active && this._compileQueue.length) {
        ...
        if (args[0] === {}) {
            ...
            this._layout(args[0], function (error, files, data) {
                ...
                that.info('Finished %s sprite compilation', Object.keys(data).map(function (mode) {
                    return '«' + mode + '»';
                }).join(' + '));
...
SVGSpriter.prototype._logStats = function (files) {
    ...
    this.info('Created %s', Object.entries(exts).map(function (ext) {
...
SVGSpriter.prototype._closeBrowser = function () {
